...
### 默认后端配置变更
**重要更新**：CUDA后端现在是默认启用的硬件后端配置。

- **默认启用**：在CMakeLists.txt中，SD_CUDA选项默认值为ON，这意味着在没有明确指定的情况下，构建系统会自动启用CUDA后端支持。
- **编译时行为**：当SD_CUDA=ON时，构建系统会设置GGML_CUDA=ON并添加-DSD_USE_CUDA定义，从而在编译时启用CUDA后端。
- **运行时选择**：在运行时，init_backend()函数会按照SD_USE_CUDA、SD_USE_METAL、SD_USE_VULKAN、SD_USE_OPENCL、SD_USE_SYCL的顺序检查宏定义，CUDA后端由于默认启用而具有最高优先级。
...
- **默认启用**：现在CUDA后端是默认启用的硬件后端，无需额外配置即可获得GPU加速。
...
### 后端选择与绑定策略
- **默认后端优先**：现在CUDA后端具有最高优先级，因为它默认启用；如果需要其他后端，需要显式禁用CUDA。
...
## 性能考量
- 后端选择建议
  - **NVIDIA GPU**：现在CUDA后端是默认选择，性能与生态成熟度最佳。
...
- 后端初始化失败
  ...
  - **默认CUDA后端**：如果CUDA后端初始化失败，系统将尝试其他后端；如果所有硬件后端均失败，系统将回退至CPU后端。
- 显存不足
...
## 结论
稳定扩散.cpp通过ggml后端抽象层实现了对多硬件后端的统一支持。**现在CUDA后端是默认启用的硬件后端配置**，为大多数用户提供即插即用的GPU加速体验。用户可通过编译期宏与运行时参数灵活选择后端，结合内存与调度策略在不同平台上获得稳定且高效的推理体验。建议依据硬件生态与平台特性选择最适合的后端，并配合性能文档与故障排除指南进行优化与排障。

## 附录

### 后端选择与配置速查
- **默认配置**：CUDA后端现在默认启用（SD_CUDA=ON），无需额外配置即可获得GPU加速。
- 宏开关
  - SD_USE_CUDA：启用CUDA后端（默认已启用）
